...
export function ShipperBookingPicker({
  ...
  const filtered = useMemo(() => {
    ...
    return bookings.filter((row) =>
      [row.bookingReference, row.customer, row.shipper, row.portOfLoading, row.portOfDischarge, row.status]
        .join(" ")
        .toLowerCase()
        .includes(q)
    );
  }, [bookings, queryReady, trimmedQuery]);
  ...
  return (
    <div className="carrier-sb-picker">
      <div className="field">
      ...
      {!queryReady ? (
        <p className="panel__muted carrier-sb-picker__empty">
          {t("carrierBookingsPage.searchShipperBookingsHint")}
        </p>
      ) : bookings.length === 0 ? (
